...
namespace DurableFunctionsMonitor.DotNetBackend
{
    internal static class Auth
    {
        ...
        public static async Task ValidateIdentityAsync(ClaimsPrincipal principal, IHeaderDictionary headers, IRequestCookieCollection cookies, string taskHubName)
        {
            ...
            var roleClaims = DfmEndpoint.Settings.AllowedAppRoles == null
                ? Enumerable.Empty<Claim>()
                : principal.FindAll(RolesClaim);

            var groupClaims = DfmEndpoint.Settings.AllowedGroups == null
                ? Enumerable.Empty<Claim>()
                : principal.FindAll(GroupsClaim);

            if (
                !roleClaims.Any(claim => DfmEndpoint.Settings.AllowedAppRoles.Contains(claim.Value)) &&
                !groupClaims.Any(claim => DfmEndpoint.Settings.AllowedGroups.Contains(claim.Value))
            )
            {
                throw new UnauthorizedAccessException($"User {userNameClaim.Value} doesn't have any of roles or groups mentioned in {EnvVariableNames.DFM_ALLOWED_APP_ROLES} or {EnvVariableNames.DFM_ALLOWED_GROUPS} config settings. Call is rejected");
            }
        }
